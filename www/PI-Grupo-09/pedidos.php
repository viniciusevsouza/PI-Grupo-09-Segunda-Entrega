...
<tr>
    ...
    <th>Status</th>
    <th>
        <form action="cadastrarPedido.php" method="post" class="form-inline">
            ...
            foreach ($pedidos as $p) {
                ...
                $hoje = new DateTime();
                $dataEntrega = new DateTime($p['data_entrega']);

                $diferenca = $hoje->diff($dataEntrega)->days;

                if ($dataEntrega >= $hoje && $diferenca <= 3 && $p['status'] != "Finalizado") {
                    $classe = 'alerta';
                }
